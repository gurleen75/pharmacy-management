...
if ( !$connection ) {
    ...
    if ( 'addManager' == $action ) {
        ...
        if ( $medname && $batch_no && $cost ) {
            $query = "INSERT INTO medicines(medname,batch_no,cost) VALUES ('{$medname}','$batch_no','$cost')";
            mysqli_query( $connection, $query );
        ...
        if ( $medname && $batch_no && $cost ) {
            $query = "UPDATE medicines SET medname='{$medname}', batch_no='{$batch_no}', cost='$cost' WHERE id='{$id}'";
            mysqli_query( $connection, $query );
            header( "location:index.php?id=allMedicine" );
        }
